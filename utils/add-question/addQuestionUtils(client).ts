  ...
  z
    .object({
      ...
      answer: z.union([
        ...
        z.instanceof(File).refine((file) => ['image/jpeg', 'image/png'].includes(file.type), {
...
export const contentTypeSchema = z.union([textQuestionPartSchema, imageQuestionPartSchema]);
...
export function processQuestionPartIntoQuestionContentJSON(
  ...
  questionContentJSON.questionContent.root = questionContentJSON.questionContent.root.map(
    ({ order, ...rest }) => rest,
  );
  ...
  Object.keys(questionContentJSON.questionContent.indexed).forEach((idx) => {
    Object.keys(questionContentJSON.questionContent.indexed[idx]).forEach((subIdx) => {
      ...
      questionContentJSON.questionContent.indexed[idx][subIdx] =
        questionContentJSON.questionContent.indexed[idx][subIdx].map(({ order, ...rest }) => rest);
    });
  });
  ...
  Object.keys(sortedTempLeafSets).forEach((key) => {
    questionContentJSON.questionLeafs![key] = Array.from(sortedTempLeafSets[key]).sort(
...
export function questionAnswerRequiresReset(
  ...
  if (questionType === 'MCQ') {
    ...
    return (
      !firstObj ||
      !('questionIdx' in firstObj && 'questionSubIdx' in firstObj && 'answer' in firstObj)
    );
  }
  return false;
}
...
export function createQuestionAnswerValueAfterReset(
  ...
  if (questionType === 'MCQ') {
    ...
    return [{ options: [], answer: [], mark: '2' }] as z.infer<typeof MCQAnswerSchema>;
  } else if (questionType === 'OEQ') {
  ...
  if (questionType === 'MCQ') {
    ...
    if (!questionLeafs) {
      ...
      const foundObj = questionAnswer.find(
        (obj) => obj.questionIdx === 'root' && obj.questionSubIdx === 'root',
      ) as
        | {
            questionIdx: string;
  ...
  for (const part of questionPart) {
    ...
    if (!isText) {
      ...
      if (error) {
        ...
        const { image, ...restOfPart } = part as z.infer<typeof imageQuestionPartSchema>;
