...
export function processQuestionPartIntoFinalQuestionContentQuestionLeafs(
  ...
  questionParts.forEach((questionPart) => {
    ...
    if (questionIdx === 'root') {
      ...
      questionContentWithOrderJSON.indexed[questionIdx][questionSubIdx].push(contentItem);
    }
  ...
  finalQuestionContent.root = questionContentWithOrderJSON.root.map(({ order, ...rest }) => rest);
  Object.keys(questionContentWithOrderJSON.indexed).forEach((idx) => {
    Object.keys(questionContentWithOrderJSON.indexed[idx]).forEach((subIdx) => {
      ...
      questionContentWithOrderJSON.indexed[idx][subIdx].sort((a, b) => a.order! - b.order!);
      finalQuestionContent.indexed[idx][subIdx] = questionContentWithOrderJSON.indexed[idx][
        subIdx
      ].map(({ order, ...rest }) => rest);
    });
  });
  ...
  Object.keys(sortedTempLeafSets).forEach((key) => {
    questionLeafs[key] = Array.from(sortedTempLeafSets[key]).sort(
